import { isNegativeStat } from "@/sim/organs";
...
export function StatBar({ label, value, inverted }: Props) {
  ...
  const isNeg = inverted ?? isNegativeStat(label);
  const good = isNeg ? v < 25 : v > 70;
  const bad = isNeg ? v > 65 : v < 35;
  const color = bad ? "var(--crisis)" : good ? "var(--health)" : "var(--warning)";
  return (
    <div className="space-y-1">
      <div className="flex items-baseline justify-between text-[10px] uppercase tracking-[0.15em]">
        <span className="text-muted-foreground">{label}</span>
        <span style={{ color }} className="font-semibold tabular-nums">{v.toFixed(0)}</span>
      </div>
